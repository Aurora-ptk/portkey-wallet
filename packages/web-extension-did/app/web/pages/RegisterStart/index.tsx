...
import { CreateWalletType, DIDWalletInfo, IGuardianIdentifierInfo, SignUpAndLogin } from '@portkey/did-ui-react';
...
export default function RegisterStart() {
  ...
  const onSuccess = useCallback(
    async (info: IGuardianIdentifierInfo) => {
      signInSuccessRef.current = info;
      if (info.isLoginGuardian && createType.current !== 'Login') {
        setOpen(true);
        setLoading(false);
        return;
      }
      if (!info.isLoginGuardian && createType.current !== 'SignUp') {
        setOpen(true);
        setLoading(false);
        return;
      }
    ...
    <div id="register-start-wrapper">
      ...
      <LoginModal
        ...
        onConfirm={() => {
          ...
          createType.current = signInSuccessRef.current.isLoginGuardian ? 'Login' : 'SignUp';
          finished();
        }}
      />
    </div>
